#!/usr/bin/env bash
# coherd 安装脚本
#   bin/coherd        → ~/.local/bin/coherd   (已有则备份为 coherd.bak, 覆盖式只留一份)
#   roles/CONTRACT.md + 4 per-role → ~/.config/coherd/ (已有则各自备份, 只留一份 .bak)
...
if [ -e "$BIN_DIR/coherd" ] && [ ! -L "$BIN_DIR/coherd" ]; then
  rm -f "$BIN_DIR/coherd.bak" # 覆盖式: 只保留最新一份, 不堆积时间戳备份
  cp "$BIN_DIR/coherd" "$BIN_DIR/coherd.bak"
...
mkdir -p "$CFG_DIR"
_install_doc() { # 文件名 → 装到 $CFG_DIR (已有则备份后覆盖; .bak 覆盖式只留一份)
  local f="$1"
  [ -f "$HERE/roles/$f" ] || die "缺失 $HERE/roles/$f"
  if [ -e "$CFG_DIR/$f" ]; then
    rm -f "$CFG_DIR/$f.bak" # 覆盖式: 只保留最新一份, 不堆积时间戳备份
    cp "$CFG_DIR/$f" "$CFG_DIR/$f.bak"
...
if [ -d "$HERE/skills" ]; then
  ...
  for sk in "$HERE"/skills/*; do
    ...
    if [ -e "$CLAUDE_SKILLS/$name" ] && [ ! -L "$CLAUDE_SKILLS/$name" ]; then
      rm -f "$CLAUDE_SKILLS/$name.bak" # 覆盖式: 只保留最新一份, 不堆积时间戳备份
      mv "$CLAUDE_SKILLS/$name" "$CLAUDE_SKILLS/$name.bak"
      info "已备份原 $CLAUDE_SKILLS/$name → $name.bak"
    fi
