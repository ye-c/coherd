...
class ReplayTest(unittest.TestCase):
    ...
    def test_reverse_clears_pending(self):
        ...
        self.assertEqual(
            l.owner("w2p", "executor"), "reviewer"
        )  # 新欠：executor 欠 reviewer
    ...
    def test_bad_lines_skipped(self):
        ...
        log.write_text(
            '{"op":"send","ws":"w2p","from":"coordinator","to":"reviewer","msg_id":"1","ts":"t"}\nnot json\n',
            encoding="utf-8",
        )
        l = W.Ledger()
        l.replay(log, 0)
    ...
    def test_no_reply_reply_still_clears_pending(self):
        ...
        self.assertIsNone(
            l.owner("w2p", "executor")
        )  # notify（expect_reply=false）不产生新欠
...
class SocketFallbackTest(unittest.TestCase):
    ...
    def test_env_missing_status_ok_resolves_socket(self):
        proc = mock.Mock(
            returncode=0, stdout=json.dumps({"server": {"socket": "/fake/herdr.sock"}})
        )
        with (
            self._env_missing_socket(),
    ...
    def test_env_missing_status_exception_keeps_none(self):
        with (
            self._env_missing_socket(),
            mock.patch("coherd.watch.subprocess.run", side_effect=FileNotFoundError),
        ):
            w = W.Watch(ws="w2v")
        self.assertIsNone(w.socket_path)
        with self.assertRaises(RuntimeError):
